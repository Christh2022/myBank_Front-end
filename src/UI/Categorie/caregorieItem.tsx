...
export const CategorieItem = ({ category }: { category: Category }) => {
    const data = transactionCategories.filter((item)=>item.label === category.icon_name)
  const Icon = data[0].icon;
  return (
    <div className=" flex flex-row items-center justify-start px-5 h-[80px] gap-3.5 bg-[rgba(255,255,255,0.1)] rounded-[6px]  ">
      <span className=" w-[65px] h-[65px] bg-[rgba(252,163,17,0.28)] flex justify-center rounded-full text-[30px] items-center ">
        <Icon className="" />
      </span>
      <div className=" flex flex-col gap-1 ">
        <h6 className="font-medium text-[15px]  ">{category.title}</h6>
